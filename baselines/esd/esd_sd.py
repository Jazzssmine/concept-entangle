...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', help='Path to save model', type=str, default='/work/hdd/bcxt/anon3/unlearn_diff/esd/esd-models/sd/')
    ...
    # Construct filename
    filename = f"esd-{erase_concept.replace(' ', '_')}-from-{erase_concept_from.replace(' ', '_')}-{train_method.replace('-','')}.safetensors"
    filepath = os.path.join(save_path, filename)
    

    save_file(esd_param_dict, filepath)
    print(f"Successfully saved to: {filepath}")
    print(f"File exists after save: {os.path.exists(filepath)}")
"""
python esd_sd.py   --erase_concept "bicycle"   --train_method "esd-x"
"""