...
```bash
python scripts/gather_sae_acts_ca_prompts.py --checkpoint_path "sae-ckpts" --hookpoint "unet.up_blocks.1.attentions.1" --pipe_path "saeuron/style50" --save_dir "saeuron/style_activations"

python scripts/gather_sae_acts_ca_prompts.py \
    --checkpoint_path "sae-ckpts" \
    --hookpoint "unet.up_blocks.1.attentions.2" \
    --pipe_path "diff_unlearn/saeuron/style50" \
    --save_dir "saeuron/style_activations"
```
...
--pipe_path "/projects/bcxt/diff_unlearn/saeuron/style50" \
--save_dir "/projects/bcxt/diff_unlearn/saeuron/object_activations"
```
...
```bash
accelerate launch --num_processes 1 scripts/sweep_cls_distr.py --percentiles [99.99,99.995,99.999] --multipliers [-1.0,-5.0,-10.0,-15.0,-20.0,-25.0,-30.0]> --seed 42 --output_dir '/projects/bcxt/diff_unlearn/saeuron/sweep' --pipe_checkpoint '/projects/bcxt/diff_unlearn/saeuron/style50' --hookpoint 'unet.up_blocks.1.attentions.1' --class_latents_path '/projects/bcxt/diff_unlearn/saeuron/object_activations/cls_latents_dict_unet.up_blocks.1.attentions.1.pkl' --sae_checkpoint 'sae-ckpts' --steps 100

accelerate launch --num_processes 4 scripts/sweep_cls_distr.py \
  --percentiles [99.99,99.995,99.999] \
  --multipliers [-1.0,-5.0,-10.0,-15.0,-20.0,-25.0,-30.0] \
  --seed 42 \
  --output_dir '/projects/bcxt/diff_unlearn/saeuron/sweep' \
  --pipe_checkpoint '/projects/bcxt/diff_unlearn/saeuron/style50' \
  --hookpoint 'unet.up_blocks.1.attentions.2' \
  --class_latents_path '/projects/bcxt/diff_unlearn/saeuron/object_activations/cls_latents_dict_unet.up_blocks.1.attentions.1.pkl' \
  --sae_checkpoint 'sae-ckpts' \
  --steps 100
```

### sweep for custom class
accelerate launch --num_processes 1 scripts/sweep_cls_distr.py \
  --percentiles='[99.99,99.995,99.999]' \
  --multipliers='[-1.0,-5.0,-10.0,-15.0,-20.0,-25.0,-30.0]' \
  --seed 42 \
  --output_dir '/projects/bcxt/diff_unlearn/saeuron/sweep' \
...
python scripts/run_acc_all_cls_sweep.py \
  --percentiles [99.99,99.995,99.999] \
  --multipliers [-1.0,-5.0,-10.0,-15.0,-20.0,-25.0,-30.0] \
  --input_dir_base '/projects/bcxt/diff_unlearn/saeuron/sweep' \
  --output_dir_base '/projects/bcxt/diff_unlearn/saeuron/object_output' \
  --class_ckpt /projects/bcxt/diff_unlearn/saeuron/cls_model/style50.pth \
  --batch_size 32 --seed 42
```

And find best parameters for each object:
```bash
python scripts/find_best_params_cls_sweep.py --percentiles [99.99] --multipliers [-1.0] --base_path '/projects/bcxt/diff_unlearn/saeuron/sweep'
```
...
```bash
accelerate launch --num_processes 4 scripts/sample_unlearning_distr.py --percentile 99.999 --multiplier -1.0 --seed 42 --output_dir 'unlearn_diff/saeuron/style_unlearn' --pipe_checkpoint 'unlearn_diff/saeuron/style50' --hookpoint 'unet.up_blocks.1.attentions.2' --style_latents_path 'unlearn_diff/saeuron/style_activations/style_latents_dict_unet.up_blocks.1.attentions.2.pkl' --sae_checkpoint 'sae-ckpts' --steps 100  --batch_size 4
```

All images will be saved as `jpg` files under `<path/to/output/dir>`.

### 5.3 Object unlearning
#### DIRECTLY RUN THIS INSTEAD OF SWEEPING!
Run sampling with SAE-based unlearning during the inference:
```bash
accelerate launch --num_processes 1 scripts/sample_unlearning_cls_distr.py \
--output_dir 'unlearn_diff/saeuron/object_unlearn' \
--pipe_checkpoint 'unlearn_diff/saeuron/style50' \
--hookpoint 'unet.up_blocks.1.attentions.1' \
--class_latents_path 'unlearn_diff/saeuron/object_activations/cls_latents_dict_unet.up_blocks.1.attentions.1.pkl' \
--sae_checkpoint 'sae-ckpts' \
--target_class Horses \
--steps 100 --batch_size 4 --seed 42 

python scripts/sample_unlearning_cls_distr.py \
  --pipe_checkpoint unlearn_diff/saeuron/style50 \
  --hookpoint unet.up_blocks.1.attentions.1 \
  --class_latents_path unlearn_diff/saeuron/object_activations/cls_latents_dict_unet.up_blocks.1.attentions.1.pkl \
  --sae_checkpoint sae-ckpts \
  ...
  --multiplier -15.0 \
  --steps 50 \
  --output_dir unlearn_diff/saeuron/object_unlearn_cats
```
...
accelerate launch --num_processes 1 scripts/sample_unlearning_cls_distr.py \
  ...
  --steps 100 \
  --seed 2 \
  --output_dir 'unlearn_diff/saeuron/object_unlearn_zebra'
...
accelerate launch --num_processes 1 scripts/sample_unlearning_cls_distr.py \
  ...
  --sae_checkpoint 'sae-ckpts' \
  --target_class Horses \
  --prompts_file unlearn_diff/outputs/prompts/horse/for_generate_example/direct.csv \
  --percentile 99.99 \
  --multiplier -40.0 \
  --steps 100 \
  --seed 2 \
  --output_dir 'unlearn_diff/saeuron/object_unlearn_horses_direct'
...
--input_dir /projects/bcxt/diff_unlearn/saeuron/object_unlearn \
--style_ckpt /projects/bcxt/diff_unlearn/saeuron/cls_model/style50.pth \
--class_ckpt /projects/bcxt/diff_unlearn/saeuron/cls_model/style50_cls.pth \
--output_dir /projects/bcxt/diff_unlearn/saeuron/object_unlearn_eval \
--batch_size 4
```
