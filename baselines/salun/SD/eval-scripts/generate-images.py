...
def generate_images(
    ...
    loaded_unet_path = "CompVis/stable-diffusion-v1-4 (base unet)"

    # model_name can be an absolute path or a short name (not starting with "/").
    # The original check `"SD" not in model_name` was intended to skip loading when
    # using the base SD model, but it also fires on any absolute path that contains
    # "/SD/" (e.g. .../salun/SD/models/...), silently leaving the base weights in place.
    if model_name not in ("SD", "SD14", "CompVis/stable-diffusion-v1-4"):
        try:
            if os.path.isabs(model_name):
                # Absolute path: derive the diffusers .pt file from the basename
                base = os.path.basename(model_name)
                model_path = os.path.join(
                    model_name, base.replace("compvis", "diffusers") + ".pt"
                )
            else:
                model_path = (
