...
def _resolve_local_hf_snapshot(model_id):
    """Resolve a local HuggingFace cache snapshot path if available."""
    if os.path.isdir(model_id):
        return model_id

    safe_name = model_id.replace("/", "--")
    # Support explicit cache roots and common defaults used by HF/transformers.
    cache_roots = []
    ...
    if huggingface_hub_cache:
        cache_roots.append(huggingface_hub_cache)
    if hf_home:
        # HF_HOME usually stores hub cache in HF_HOME/hub
        cache_roots.extend([hf_home, os.path.join(hf_home, "hub")])
    if transformers_cache:
        cache_roots.append(transformers_cache)
    cache_roots.append(os.path.expanduser("~/.cache/huggingface/hub"))

    # Deduplicate while preserving order.
    cache_roots = [p for p in dict.fromkeys(cache_roots) if p]
    for root in cache_roots:
        root_path = Path(root)
        if not root_path.exists():
            continue

        # Newer huggingface-hub layout.
        repo_dir = root_path / f"models--{safe_name}"
        ...
        # Older transformers layout can store direct folders with files.
        direct_repo_dir = root_path / model_id
        if direct_repo_dir.is_dir():
            return str(direct_repo_dir)
    return None


def _from_pretrained_with_local_cache(cls, version):
    """Load from local HF cache first, then fallback to remote."""
    local_path = _resolve_local_hf_snapshot(version)
    if local_path is not None:
        return cls.from_pretrained(local_path, local_files_only=True)
    try:
        return cls.from_pretrained(version)
    except OSError as exc:
        raise OSError(
            f"Unable to load '{version}' with {cls.__name__}. "
            "No local cache snapshot was found and remote download failed. "
            "If running offline, pre-download this model/tokenizer with "
            "`huggingface-cli download openai/clip-vit-large-patch14` "
            "or set HF cache env vars to a valid local cache."
        ) from exc
...
class FrozenT5Embedder(AbstractEncoder):
    ...
    def __init__(
        ...
        super().__init__()
        self.tokenizer = _from_pretrained_with_local_cache(T5Tokenizer, version)
        self.transformer = _from_pretrained_with_local_cache(T5EncoderModel, version)
        self.device = device
...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(
        ...
        super().__init__()
        self.tokenizer = _from_pretrained_with_local_cache(CLIPTokenizer, version)
        self.transformer = _from_pretrained_with_local_cache(CLIPTextModel, version)
        self.device = device
        self.max_length = max_length  # TODO: typical value?
        self.freeze()
...
class ClipImageProjector(AbstractEncoder):
    ...
    def __init__(
        ...
        super().__init__()
        self.model = _from_pretrained_with_local_cache(CLIPVisionModel, version)
        self.model.train()
        self.max_length = max_length  # TODO: typical value?
        self.antialias = True
