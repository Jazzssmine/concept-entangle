...
class LocalImageFolderDataset(Dataset):
    def __init__(self, data_path, transform=None):
        ...
        if len(self.image_paths) == 0:
            raise ValueError(
                f"No images found under {self.data_path}. "
                "Expected files with extensions: .jpg, .jpeg, .png, .bmp, .webp"
            )
...
class HORSE(LocalImageFolderDataset):
    def __init__(self, data_path="data/horse", transform=None):
        super().__init__(data_path=data_path, transform=transform)


class NOT_HORSE(LocalImageFolderDataset):
    def __init__(self, data_path="data/not-horse", transform=None):
        super().__init__(data_path=data_path, transform=transform)
...
def setup_forget_horse_data(
    ...
    forget_set = HORSE(data_path=horse_data_path, transform=transform)
    forget_dl = DataLoader(forget_set, batch_size=batch_size)

    remain_set = NOT_HORSE(data_path=not_horse_data_path, transform=transform)
    remain_dl = DataLoader(remain_set, batch_size=batch_size)
    return forget_dl, remain_dl
