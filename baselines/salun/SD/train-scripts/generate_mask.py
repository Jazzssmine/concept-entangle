...
def generate_nsfw_mask(
    ...
    with tqdm(total=len(train_dl)) as pbar:
        for i, images in enumerate(train_dl):
            ...
            forget_noisy = model.q_sample(x_start=forget_input, t=timesteps, noise=noise)

            forget_out = model.apply_model(forget_noisy, timesteps, forget_emb)
            null_out = model.apply_model(forget_noisy, timesteps, null_emb)
...
def generate_concept_mask(
    ...
    concept_prompt="a photo of a horse",
    concept_name="horse",
    image_size=512,
    num_timesteps=1000,
):
    # MODEL TRAINING SETUP
    model = setup_model(config_path, ckpt_path, device)
    train_dl, _ = setup_forget_horse_data(
        batch_size=batch_size,
        image_size=image_size,
        horse_data_path=concept_data_path,
        not_horse_data_path=not_concept_data_path,
    )

    model.eval()
    criteria = torch.nn.MSELoss()
    optimizer = torch.optim.Adam(model.model.diffusion_model.parameters(), lr=lr)
    concept_prompt = concept_prompt.strip() if concept_prompt else "a photo of a horse"
    concept_slug = concept_name.strip().replace(" ", "_") if concept_name else "concept"
    ...
    with tqdm(total=len(train_dl)) as pbar:
        for i, images in enumerate(train_dl):
            ...
            null_prompts = [""] * batch_size
            prompts = [concept_prompt] * batch_size
    ...
    with torch.no_grad():
        ...
        for i in threshold_list:
            ...
            # Calculate the threshold index for top i fraction elements
            threshold_index = int(len(all_elements) * i)
            ...
            torch.save(hard_dict, os.path.join(f"mask/{concept_slug}_{i}.pt"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--horse", help="enable horse concept mask generation", action="store_true"
    )
    parser.add_argument(
        "--horse_data_path",
        help="dataset path for horse images",
        type=str,
        required=False,
        default="data/horse",
    )
    parser.add_argument(
        "--not_horse_data_path",
        help="dataset path for non-horse images",
        type=str,
        required=False,
        default="data/not-horse",
    )
    parser.add_argument(
        "--concept",
        help="enable generic concept mask generation (uses concept prompt and custom data paths)",
        action="store_true",
    )
    parser.add_argument(
        "--concept_name",
        help="concept name used in saved mask filename",
        type=str,
        required=False,
        default="concept",
    )
    parser.add_argument(
        "--concept_prompt",
        help="text prompt used for concept mask generation",
        type=str,
        required=False,
        default=None,
    )
    parser.add_argument(
        "--concept_data_path",
        help="dataset path for target concept images",
        type=str,
        required=False,
        default=None,
    )
    parser.add_argument(
        "--not_concept_data_path",
        help="dataset path for non-target concept images",
        type=str,
        required=False,
        default=None,
    )
    ...
    if args.horse or args.concept:
        use_horse_mode = args.horse and not args.concept
        concept_name = "horse" if use_horse_mode else args.concept_name
        concept_prompt = "a photo of a horse" if use_horse_mode else (
            args.concept_prompt or f"a photo of a {concept_name}"
        )
        concept_data_path = args.horse_data_path if use_horse_mode else (
            args.concept_data_path or args.horse_data_path
        )
        not_concept_data_path = args.not_horse_data_path if use_horse_mode else (
            args.not_concept_data_path or args.not_horse_data_path
        )
        generate_concept_mask(
            c_guidance,
            batch_size,
            epochs,
            lr,
