...
def init_latent(latent, model, height, width, generator, batch_size):
    if latent is None:
        # Generator device must match the tensor PyTorch draws into (CPU vs CUDA mismatch errors otherwise).
        device = generator.device if generator is not None else torch.device("cpu")
        dtype = getattr(model.unet, "dtype", None) or model.unet.conv_in.weight.dtype
        latent = torch.randn(
