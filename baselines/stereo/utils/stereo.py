...
def search_thoroughly_enough(
    ...
    saved_tokens = {}
    # List to track adversarial prompts used across iterations
    adv_prompts = []
    ...
    for iteration in range(n_iterations):
        ...
        # Track adversarial prompts used for image generation this iteration
        iteration_prompts = []
        # Prompt used with the erased concept for this iteration
        iteration_prompts.append(f"{generic_prompt} {current_concept}")
        # Prompts used with each adversarial placeholder token discovered so far
        for token in saved_tokens.values():
            iteration_prompts.append(f"{generic_prompt} {token}")
        adv_prompts.append(iteration_prompts)
    ...
    # Save adversarial prompts to results/adv_prompts.json
    results_dir = "results"
    os.makedirs(results_dir, exist_ok=True)
    adv_prompts_path = os.path.join(results_dir, "adv_prompts.json")
    # Convert list of prompts per iteration into the requested JSON structure
    adv_prompts_dict = {f"iteration_{i}": prompts for i, prompts in enumerate(adv_prompts)}
    with open(adv_prompts_path, "w", encoding="utf-8") as f:
        json.dump(adv_prompts_dict, f, indent=2, ensure_ascii=False)
    print(f"Adversarial prompts saved to {adv_prompts_path}")
